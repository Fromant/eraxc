...
namespace eraxc::syntax {
    const std::string& syntax_analyzer::get_typename(size_t id) const {
    ...
    const std::string& syntax_analyzer::get_identifier(size_t id) const {
    ...
    errable<AST::expr_node*> syntax_analyzer::parse_expr_node(const std::vector<lexic::Token>& ts, size_t& i,
                                                              scope& scope) {
        auto t = ts[i];
        ...
        if (t.t == lexic::Token::L_BRACKET) {
            ...
            tr->isInstant = true;
            tr->data.d = std::stoll(t.data);
        }
        else if (t.t == lexic::Token::STRING_INSTANT) {
            tr->isInstant = true;
            tr->data.d = string_instants.size();
            auto r = string_instants.emplace(t.data, tr->data.d);
            if (!r.second) tr->data.d = r.first->second;
        }
    ...
    errable<std::vector<AST::statement_node>> syntax_analyzer::parse_func_body(
        const std::vector<lexic::Token>& ts, size_t& i, scope& scope) {
    ...
    errable<AST::func_node*> syntax_analyzer::parse_func(const std::vector<lexic::Token>& ts,
                                                         size_t& i,
    ...
    errable<AST::decl_node> syntax_analyzer::parse_decl(const std::vector<lexic::Token>& ts,
                                                        size_t& i,
                                                        scope& scope,
                                                        lexic::Token::type end = lexic::Token::SEMICOLON) {
        AST::decl_node tr{};
    ...
    errable<void> syntax_analyzer::parse_function_definition(const std::vector<lexic::Token>& ts, size_t& i,
                                                             scope& global_scope) {
    ...
    errable<std::vector<AST::node>> syntax_analyzer::analyze(const std::vector<lexic::Token>& ts) {
        ...
        //first iter - parse all global definition(including functions)
