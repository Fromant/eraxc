...
namespace eraxc::IL {
    ...
    error::errable<std::vector<IL_node>> IL_handler::parse_declaration(const std::vector<token> &tokens,
        ...
        if(!scope.contains_type(tokens[i].data)) return {"No such typename " + tokens[i].data, tr};
        ...
        scope.add_id(tokens[i + 1].data, type);
    ...
    error::errable<void> IL_handler::translate_function(const std::vector<token> &tokens, int &i) {
        ...
        const u64 func_id = global_scope.add_id(tokens[i + 1].data, return_type);
        ...
        while(tokens[i].t != token::R_BRACKET) {
            ...
            if(tokens[i].t == token::IDENTIFIER) {
                ...
                func_scope.add_id(tokens[i + 1].data, func_scope.get_type_id(tokens[i].data));
            } else
