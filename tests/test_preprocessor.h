...
namespace tests {
    int test_preprocessor_tokenizer() {
        ...
        std::cerr << "Runnig preprocessor tokenizer tests..." << std::endl;

        auto passed_tests = 0;

        //test 1 - #define macro, #ifdef #ifndef
        //TODO: #define macro hasnt been implemented yet
        eraxc::tokenizer t1 {};
        ...
        if (!r1) {
            ...
            if (r1.value != res1) {
                if (r1.value.size() != res1.size()) {
                    std::cerr << "Test 1 failed: sizes dont match: expected size: " << res1.size()
                              << "; got size: " << r1.value.size() << std::endl;
                } else {
        ...
        //TODO: add processing for characters ! ? :
        std::string str2 = "; ! () {} [] ? : . ,";
                ...
                for (size_t i = 0; i < res3.size(); ++i) {
                    if (r3.value[i].t != res3[i]) {
                        std::cerr << "Test 3 failed: type mismatch at position " << i
                                  << " - expected: " << static_cast<int>(res3[i])
                                  << ", got: " << static_cast<int>(r3.value[i].t) << std::endl;
                        is_passed = false;
                        break;
                    }
                }
